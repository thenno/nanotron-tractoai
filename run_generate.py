...
def main():
    ...
    if AutoTokenizer is not None:
        ...
        dummy_inputs = [
            ...
            "Since ChatGPT launched, there has been a ton of research into prompt engineering. There are now several established design principles, like including specific instructions, demonstrating in-context examples, and using chain-of-thought prompting. These principles do really work, but they lead to longer prompts, which can be harder to handle. Additionally, minor modifications to a prompt, can significantly impact output quality. The longer the prompt, the more variables. This highlights the importance of having a prompt management tool that allows for rigorous testing. Previous automatic prompt optimization studies focused on short prompts. The methods focus on replacing specific words or using the LLM to rewrite the entire prompt. These methods don't translate well to long prompts because:",
            "YTsaurus is a distributed storage and processing platform for large amounts of data. It includes MapReduce computation model, a distributed file system and a NoSQL key-value storage. This section contains information about the basic features and structure of the YTsaurus system. A YTsaurus cluster is a group of instances located in containers on physical hosts and operating as a unit. Based on the tasks to be solved, clusters can be divided into: Computing clusters for massive parallel big data processing with MapReduce operations. Dynamic table clusters (key-value storage). Geodistributed clusters for coordination. A YTsaurus computing cluster is able:",
        ]
        ...
        outputs = decode_text(
            ...
            generation_config=GenerationArgs(sampler=SamplerType.TOP_K),
